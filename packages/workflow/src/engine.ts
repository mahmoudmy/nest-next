import type { Transaction, Task, WorkflowInstance, WorkflowStep } from '@qms/database';
...
/** Call commands inside the caller's serializable transaction. This engine owns execution state. */
export class WorkflowEngine {
  ...
  async createWorkflowVersion(tx:Transaction,ctx:TenantContext,workflowId:string) {
    ...
    await audit(tx,ctx,'WORKFLOW_VERSION_CREATED','workflow',workflowId,'SUCCESS',{version:row.number}); return row;
  }
  async addStep(tx:Transaction,ctx:TenantContext,versionId:string,input:StepInput) {
    ...
    await audit(tx,ctx,'WORKFLOW_STEP_ADDED','workflow',version.workflowId); return row;
  }
  async addTransition(tx:Transaction,ctx:TenantContext,versionId:string,input:TransitionInput) {
    const version=await this.repository.version(tx,ctx,versionId); assertDraft(version.status);
    ...
    await audit(tx,ctx,'WORKFLOW_TRANSITION_ADDED','workflow',version.workflowId); return row;
  }
  async publishWorkflow(tx:Transaction,ctx:TenantContext,versionId:string) {
    ...
    await audit(tx,ctx,'WORKFLOW_PUBLISHED','workflow',version.workflowId,'SUCCESS',{version:version.number}); return row;
  }
  async startWorkflow(tx:Transaction,ctx:TenantContext,input:StartWorkflowInput) {
    ...
    await audit(tx,ctx,'WORKFLOW_STARTED','workflow-instance',instance.id); return instance;
  }
  ...
  getWorkflowInstance(tx:Transaction,ctx:TenantContext,id:string) { return this.repository.instance(tx,ctx,id); }
  async getCurrentStep(tx:Transaction,ctx:TenantContext,id:string) { return (await this.repository.instance(tx,ctx,id)).steps.filter(s=>s.status==='IN_PROGRESS'); }
  /** Only the task engine calls this, in the same transaction as task completion. */
  async completeWorkflowStep(tx:Transaction,ctx:TenantContext,task:Task,outcome:string) {
    ...
    await audit(tx,ctx,'WORKFLOW_STEP_COMPLETED','workflow-instance',instance.id,'SUCCESS',{outcome});
    if (nextId) {
      const next=instance.version.steps.find(s=>s.id===nextId); if (!next) throw new DomainError('CONFLICT','Invalid definition');
      await tx.workflowInstance.update({where:{id:instance.id},data:{revision:{increment:1}}});
      await this.activate(tx,ctx,instance,next);
    } else {
      await tx.workflowInstance.update({where:{id:instance.id},data:{status:'COMPLETED',completedAt:new Date(),revision:{increment:1}}});
      await audit(tx,ctx,'WORKFLOW_COMPLETED','workflow-instance',instance.id);
    }
  }
  async cancelWorkflow(tx:Transaction,ctx:TenantContext,id:string,rejected=false) {
    ...
    await this.registry.resolve(tx,ctx,instance,'workflow');
    await this.tasks.cancelForInstance(tx,ctx,id);
    ...
    await audit(tx,ctx,rejected?'WORKFLOW_REJECTED':'WORKFLOW_CANCELLED','workflow-instance',id); return row;
  }
}
