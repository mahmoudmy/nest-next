import { database,atomic,type Transaction,type WorkflowStep,type WorkflowTransition } from '@qms/database';
import { audit } from '@qms/database/dist/audit';
import { WorkflowEngine,WorkflowRepository,ResourceRegistry,type WorkflowDefinition,type WorkflowExecution } from '@qms/workflow';
...
resourceRegistry.register('PLATFORM_SANDBOX',{async resolve(tx,ctx,id){
  if(id===ctx.organizationId){const organization=await tx.organization.findFirst({where:{id,active:true,deletedAt:null}});if(organization)return{organizationId:organization.id,siteId:null};}
  ...
  if(!site)throw new DomainError('NOT_FOUND','Resource not found');return{organizationId:site.organizationId,siteId:site.id};
}});
...
export class PlatformApplication {
  private async command<T>(ctx:TenantContext,operation:(tx:Transaction)=>Promise<T>):Promise<T>{
    try{return await atomic(operation);}catch(error){if(error instanceof DomainError&&error.code==='FORBIDDEN')await audit(database,ctx,'PERMISSION_DENIED','platform',null,'DENIED');throw error;}
  }
  ...
  workflow(ctx:TenantContext,id:string):Promise<WorkflowDefinition>{return workflowRepository.definition(database,ctx,id);}
  ...
  step(ctx:TenantContext,id:string,input:StepInput):Promise<WorkflowStep>{return this.command(ctx,tx=>workflowEngine.addStep(tx,ctx,id,input));}
  transition(ctx:TenantContext,id:string,input:TransitionInput):Promise<WorkflowTransition>{return this.command(ctx,tx=>workflowEngine.addTransition(tx,ctx,id,input));}
  publish(ctx:TenantContext,id:string){return this.command(ctx,tx=>workflowEngine.publishWorkflow(tx,ctx,id));}
  archiveWorkflow(ctx:TenantContext,id:string){return this.command(ctx,async tx=>{const workflow=await workflowRepository.definition(tx,ctx,id);const row=await tx.workflow.update({where:{id},data:{deletedAt:new Date(),status:'ARCHIVED'}});await audit(tx,{...ctx,siteId:workflow.siteId},'WORKFLOW_ARCHIVED','workflow',id);return row;});}
  ...
  instance(ctx:TenantContext,id:string):Promise<WorkflowExecution>{return workflowEngine.getWorkflowInstance(database,ctx,id);}
  instances(ctx:TenantContext){return database.workflowInstance.findMany({where:scope(ctx),orderBy:{createdAt:'desc'},take:100});}
  currentSteps(ctx:TenantContext,id:string):Promise<WorkflowExecution['steps']>{return workflowEngine.getCurrentStep(database,ctx,id);}
